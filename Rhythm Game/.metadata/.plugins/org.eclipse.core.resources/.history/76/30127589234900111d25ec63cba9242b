...
public class Game extends Thread {
	...
	private int score = 0; 
	private int combo = 0; // 현재 연속 콤보수
	private int maxCombo = 0; // 게임 중 기록한 최대 콤보 (결과창용)
	...
	public void screenDraw(Graphics2D g) {
		...
		for (int i = 0; i < noteList.size(); i++) {
			...
			if (combo >= 1) { // 1콤보 이상일 때만 표시
		        g.setColor(Color.WHITE);
		        g.setFont(new Font("Steamer", Font.BOLD, 60)); // 콤보용 폰트 설정
		        g.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
		        
		        String comboText = combo + " COMBO";
		        // 텍스트 중앙 정렬을 위해 x좌표 조정 (화면 너비에 따라 조절하세요)
		        g.drawString(comboText, 850, 550); 
		    }

			if(note.getY() > 1040) { 
				judgeEvent("Miss");
			    score -= 50; // 노트를 놓치면 50점 감점
	    ...
	    // score 변수를 7자리 숫자로 포맷팅하여 출력 (예: 0000500)
	...
	public void judgeEvent(String judge) {
	    if (!judge.equals("None")) {
	        ...
	        if (judge.equals("Miss")) {
	            combo = 0; // Miss 나면 콤보 초기화
	        } else {
	            combo++; // Perfect, Great, Good, Early, Late 모두 콤보 증가
	            if (combo > maxCombo) maxCombo = combo; // 최고 콤보 기록 갱신

	            // 판정 기본 점수 계산
	            int baseScore = 0;
	            if (judge.equals("Perfect")) baseScore = 500;
	            else if (judge.equals("Great")) baseScore = 300;
	            else if (judge.equals("Good")) baseScore = 200;
	            else if (judge.equals("Early") || judge.equals("Late")) baseScore = 100;

	            // [보너스] 점수 = 기본 점수 + (콤보 * 보너스 배율)
