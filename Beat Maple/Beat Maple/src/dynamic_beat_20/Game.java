...
public class Game extends Thread {
	...
	public void run() {
		...
		// 노래가 끝날 때까지 스레드를 유지하며 대기합니다.
		while(gameMusic.isAlive()) {
			try {
				Thread.sleep(100);
	...
	public void judgeEvent(String judge) {
	    if (!judge.equals("None")) {
	        ...
	        if (judge.equals("Miss")) {
	            combo = 0;
	            countMiss++; // Miss 카운트 증가
	        } else {
	            combo++; 
	            if (combo > maxCombo) maxCombo = combo; 

	            int baseScore = 0;
	            if (judge.equals("Perfect")) { baseScore = 500; countPerfect++; } // Perfect 카운트
	            else if (judge.equals("Great")) { baseScore = 300; countGreat++; }  // Great 카운트
	            else if (judge.equals("Good")) { baseScore = 200; countGood++; }    // Good 카운트
